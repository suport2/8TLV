...
// Cashflow 25 anys — amb degradació de producció i manteniment anual
const cashflow = [-costInstal];
for (let n=1; n<=VIDA_UTIL; n++) {
  const factDeg = Math.pow(1 - DEGRADACIO, n-1); // producció decreix 0.4%/any
  const estalvi_n = estalviAny1 * factDeg * Math.pow(1+INFLACIO, n-1) - mantAnual;
  cashflow.push(estalvi_n);
}

// Cost de producció €/kWh (kWh totals 25 anys amb degradació)
const kwhTotals25 = Array.from({length: VIDA_UTIL}, (_,n) =>
  t_pv * Math.pow(1-DEGRADACIO, n)).reduce((a,b)=>a+b,0);
