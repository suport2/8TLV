// SOLENVER — Motor fotovoltaic v5 — Simulació hora·hora exacta
...
if (teFactura) {
  ...
  const mesConsum = mesClaus.map(m => parseFloat(consumos[m] || 0));
  cp1 = mesConsum.map(v => v * 0.21);
  cp2 = mesConsum.map(v => v * 0.22);
  cp3 = mesConsum.map((v,i) => v - cp1[i] - cp2[i]);
} else {
  const cAnual = parseFloat(input.consum_anual || 6792);
  cp1 = dies_l.map(d => cAnual*(d/365)*0.21);
  cp2 = dies_l.map(d => cAnual*(d/365)*0.22);
  cp3 = dies_l.map(d => cAnual*(d/365)*0.57);
}
...
const costInstal = subtotal*(1+MARGE)*(1+IVA);

// Cashflow 25 anys
const cashflow = [-costInstal];
for (let n=1; n<=VIDA_UTIL; n++)
  cashflow.push(estalviAny1 * Math.pow(1+INFLACIO, n-1));
...
return [{json:{
  ...
  co2_estalviat_kg: Math.round(t_ac*0.321),
